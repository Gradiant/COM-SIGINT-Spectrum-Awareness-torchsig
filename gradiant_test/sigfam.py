...
class Sig53:
    ...
    def __getitem__(self, idx: int) -> Tuple[np.ndarray, int]:
        ...
        if self.use_signal_data:
            ...
            # Ensure target is in correct format (e.g., extract the relevant part if it's a list or tuple)
            if isinstance(target, list) or isinstance(target, tuple):
                target = target[0]  # Assuming first element is the target label

            return sig_iq_data, target

        np_data: np.ndarray = self.T(iq_data)  # type: ignore
        target = self.TT(mod)  # type: ignore

        # If target is an int, no need to subscript
        if isinstance(target, list) or isinstance(target, tuple):
            target = target[0]  # Assuming first element is the target label
